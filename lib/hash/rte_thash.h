			...
			union rte_thash_tuple *targ)
{
	...
	*(__m128i *)&targ->v6.dst_addr =
	...
	uint8x16_t ipv6 = vld1q_u8(orig->src_addr.s6_addr);
	vst1q_u8(targ->v6.src_addr.s6_addr, vrev32q_u8(ipv6));
	ipv6 = vld1q_u8(orig->dst_addr.s6_addr);
	vst1q_u8(targ->v6.dst_addr.s6_addr, vrev32q_u8(ipv6));
#else
	int i;
	for (i = 0; i < 4; i++) {
