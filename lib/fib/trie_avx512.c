...
transpose_x16(const struct in6_addr *ips,
...
static __rte_always_inline void
transpose_x8(const struct in6_addr *ips,
...
static __rte_always_inline void
trie_vec_lookup_x16x2(void *p, const struct in6_addr *ips,
...
static void
trie_vec_lookup_x8x2_8b(void *p, const struct in6_addr *ips,
	uint64_t *next_hops)
{
	struct rte_trie_tbl *dp = (struct rte_trie_tbl *)p;
	const __m512i zero = _mm512_set1_epi32(0);
...
void
rte_trie_vec_lookup_bulk_2b(void *p, const struct in6_addr *ips,
...
void
rte_trie_vec_lookup_bulk_4b(void *p, const struct in6_addr *ips,
	uint64_t *next_hops, const unsigned int n)
{
	uint32_t i;
	for (i = 0; i < (n / 32); i++) {
...
void
rte_trie_vec_lookup_bulk_8b(void *p, const struct in6_addr *ips,
	uint64_t *next_hops, const unsigned int n)
{
	uint32_t i;
	for (i = 0; i < (n / 16); i++) {
