...
get_tbl24_idx(const struct in6_addr *ip)
{
	return ip->s6_addr[0] << 16|ip->s6_addr[1] << 8|ip->s6_addr[2];
}

static inline void *
get_tbl24_p(struct rte_trie_tbl *dp, const struct in6_addr *ip, uint8_t nh_sz)
{
	uint32_t tbl24_idx;
...
{									\
	...
	for (i = 0; i < n; i++) {					\
		...
		while (is_entry_extended(tmp)) {			\
			tmp = ((type *)dp->tbl8)[ips[i].s6_addr[j++] +	\
