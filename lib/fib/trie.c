...
get_idx(const struct in6_addr *ip, uint32_t prev_idx, int bytes, int first_byte)
{
	int i;
	uint32_t idx = 0;
	uint8_t bitshift;
...
static inline int
build_common_root(struct rte_trie_tbl *dp, const struct in6_addr *ip,
...
static int
install_to_dp(struct rte_trie_tbl *dp, const struct in6_addr *ledge,
	const struct in6_addr *r, uint64_t next_hop)
{
	...
	int common_bytes;
	int llen, rlen;
	struct in6_addr redge;

	/* decrement redge by 1*/
	redge = *r;
	for (i = 15; i >= 0; i--) {
		redge.s6_addr[i]--;
		if (redge.s6_addr[i] != 0xff)
			break;
	}

	for (common_bytes = 0; common_bytes < 15; common_bytes++) {
		if (ledge->s6_addr[common_bytes] != redge.s6_addr[common_bytes])
			break;
	}
	...
	ret = write_edge(dp, &ledge->s6_addr[first_tbl8_byte + !(common_bytes < 3)],
	...
	ret = write_edge(dp, &redge.s6_addr[first_tbl8_byte + !((common_bytes < 3))],
	...
	recycle_root_path(dp, ledge->s6_addr + TBL24_BYTES, common_tbl8, ent);
	return 0;
}

static void
get_nxt_net(struct in6_addr *ip, uint8_t depth)
{
	int i;
	uint8_t part_depth;
	uint8_t prev_byte;
	...
	prev_byte = ip->s6_addr[i];
	ip->s6_addr[i] += 1 << (8 - part_depth);
	if (ip->s6_addr[i] < prev_byte) {
		while (i > 0) {
			ip->s6_addr[--i] += 1;
			if (ip->s6_addr[i] != 0)
				break;
	...
	uint8_t depth, uint64_t next_hop)
{
	...
	do {
		...
		if (tmp != NULL) {
			...
			if (rte_rib6_is_equal(ledge.s6_addr, redge.s6_addr)) {
			 ...
			 * and wrapped around
			 */
			if (v6_addr_is_zero(ledge.s6_addr))
				break;
		} else {
			redge = *ip;
			get_nxt_net(&redge, depth);
			if (rte_rib6_is_equal(ledge.s6_addr, redge.s6_addr) &&
					!v6_addr_is_zero(ledge.s6_addr))
				break;
...
int
trie_modify(struct rte_fib6 *fib, const struct in6_addr *ip,
	uint8_t depth, uint64_t next_hop, int op)
{
	...
	for (i = 0; i < RTE_FIB6_IPV6_ADDR_SIZE; i++)
		ip_masked.s6_addr[i] = ip->s6_addr[i] & get_msk_part(depth, i);
