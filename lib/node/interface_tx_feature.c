			   ...
			   void **objs, uint16_t nb_objs)
{
	...
	while (n_left_from > 4) {
		...
		rte_node_enqueue_deferred(graph, node, next1, objs, i + 1);
		rte_node_enqueue_deferred(graph, node, next2, objs, i + 2);
		rte_node_enqueue_deferred(graph, node, next3, objs, i + 3);
		i += 4;
	}
