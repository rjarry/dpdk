		     ...
		     void **objs, uint16_t nb_objs)
{
	...
	while (n_left_from >= 4) {
		...
		rte_node_enqueue_deferred(graph, node, p_nxt[l0], objs, i);
		rte_node_enqueue_deferred(graph, node, p_nxt[l1], objs, i + 1);
		rte_node_enqueue_deferred(graph, node, p_nxt[l2], objs, i + 2);
		rte_node_enqueue_deferred(graph, node, p_nxt[l3], objs, i + 3);
		i += 4;
	}
