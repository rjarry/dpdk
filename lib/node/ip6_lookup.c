			...
			void **objs, uint16_t nb_objs)
{
	...
	while (n_left_from >= 4) {
		...
		rte_node_enqueue_deferred(graph, node, next[0], objs, i);
		rte_node_enqueue_deferred(graph, node, next[1], objs, i + 1);
		rte_node_enqueue_deferred(graph, node, next[2], objs, i + 2);
		rte_node_enqueue_deferred(graph, node, next[3], objs, i + 3);
		i += 4;
	}
