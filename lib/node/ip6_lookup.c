			...
			void **objs, uint16_t nb_objs)
{
	...
	while (n_left_from >= 4) {
		...
		rte_node_next_stream_enqueue_x4(graph, node,
			next[0], mbuf0, next[1], mbuf2, next[2], mbuf2, next[3], mbuf3);
		i += 4;
	}
	...
	while (n_left_from > 0) {
		...
		rte_node_next_stream_enqueue_x1(graph, node, next0, mbuf0);
		i += 1;
	}
...
static struct rte_node_register ip6_lookup_node = {
	...
	.nb_stream_edges = RTE_NODE_IP6_LOOKUP_NEXT_PKT_DROP + 1,
	.stream_edges = {
