			...
			void **objs, uint16_t nb_objs)
{
	...
	while (n_left_from >= 4) {
		...
		rte_node_enqueue_deferred(graph, node, result.u16[1], objs, i);
		rte_node_enqueue_deferred(graph, node, result.u16[3], objs, i + 1);
		rte_node_enqueue_deferred(graph, node, result.u16[5], objs, i + 2);
		rte_node_enqueue_deferred(graph, node, result.u16[7], objs, i + 3);
		i += 4;
	}
	...
	while (n_left_from > 0) {
		...
		rte_node_enqueue_deferred(graph, node, next0, objs, i);
		i += 1;
	}
