...
struct rte_lpm6_rule {
	struct in6_addr ip; /**< Rule IP address. */
 ...
 */
static inline void
rule_key_init(struct rte_lpm6_rule_key *key, const struct in6_addr *ip, uint8_t depth)
{
	key->ip = *ip;
	key->depth = depth;
}
...
/* Find a rule */
static int
rule_find(struct rte_lpm6 *lpm, struct in6_addr *ip, uint8_t depth,
		  uint32_t *next_hop)
{
	struct rte_lpm6_rule_key rule_key;
 ...
 */
static inline int
rule_add(struct rte_lpm6 *lpm, struct in6_addr *ip, uint8_t depth, uint32_t next_hop)
{
	int ret, rule_exist;
	struct rte_lpm6_rule_key rule_key;
	uint32_t unused;
 ...
 */
static uint32_t
get_bitshift(const struct in6_addr *ip, uint8_t first_byte, uint8_t bytes)
{
	...
	for (i = first_byte; i < (uint32_t)(first_byte + bytes); i++) {
		...
		if (bitshift < 0)
			bitshift = 0;
		entry_ind = entry_ind | ip->s6_addr[i-1] << bitshift;
	}
...
simulate_add_step(struct rte_lpm6 *lpm, struct rte_lpm6_tbl_entry *tbl,
		struct rte_lpm6_tbl_entry **next_tbl, const struct in6_addr *ip,
...
add_step(struct rte_lpm6 *lpm, struct rte_lpm6_tbl_entry *tbl,
		...
		uint32_t *next_tbl_ind, struct in6_addr *ip, uint8_t bytes,
 ...
 */
static int
simulate_add(struct rte_lpm6 *lpm, const struct in6_addr *masked_ip, uint8_t depth)
{
 ...
 */
int
rte_lpm6_add(struct rte_lpm6 *lpm, const struct in6_addr *ip, uint8_t depth,
	     uint32_t next_hop)
{
	struct rte_lpm6_tbl_entry *tbl;
	struct rte_lpm6_tbl_entry *tbl_next = NULL;
...
lookup_step(const struct rte_lpm6 *lpm, const struct rte_lpm6_tbl_entry *tbl,
		const struct rte_lpm6_tbl_entry **tbl_next, const struct in6_addr *ip,
		uint8_t first_byte, uint32_t *next_hop)
{
	...
	if ((tbl_entry & RTE_LPM6_VALID_EXT_ENTRY_BITMASK) ==
		...
		tbl8_index = ip->s6_addr[first_byte-1] +
 ...
 */
int
rte_lpm6_lookup(const struct rte_lpm6 *lpm, const struct in6_addr *ip,
		uint32_t *next_hop)
{
	...
	tbl24_index = (ip->s6_addr[0] << BYTES2_SIZE) |
			(ip->s6_addr[1] << BYTE_SIZE) | ip->s6_addr[2];
		...
		int32_t *next_hops, unsigned int n)
{
	...
	for (i = 0; i < n; i++) {
		first_byte = LOOKUP_FIRST_BYTE;
		tbl24_index = (ips[i].s6_addr[0] << BYTES2_SIZE) |
				(ips[i].s6_addr[1] << BYTE_SIZE) | ips[i].s6_addr[2];
 ...
 */
int
rte_lpm6_is_rule_present(struct rte_lpm6 *lpm, const struct in6_addr *ip, uint8_t depth,
			 uint32_t *next_hop)
{
	struct in6_addr masked_ip;
 ...
 */
static inline int
rule_delete(struct rte_lpm6 *lpm, struct in6_addr *ip, uint8_t depth)
{
	int ret;
	struct rte_lpm6_rule_key rule_key;
...
rte_lpm6_delete_bulk_func(struct rte_lpm6 *lpm,
		struct in6_addr *ips, uint8_t *depths,
		unsigned n)
{
	struct in6_addr masked_ip;
	unsigned i;
	...
	for (i = 0; i < n; i++) {
		masked_ip = ips[i];
		ip6_mask_addr(masked_ip.s6_addr, depths[i]);
 ...
 */
static int
rule_find_less_specific(struct rte_lpm6 *lpm, struct in6_addr *ip, uint8_t depth,
	struct rte_lpm6_rule *rule)
{
	...
	while (depth > 1) {
		...
		rule_key.ip.s6_addr[depth >> 3] &= mask;
 ...
 */
static void
rule_find_range(struct rte_lpm6 *lpm, const struct in6_addr *ip, uint8_t depth,
		  struct rte_lpm6_tbl_entry **from,
		  struct rte_lpm6_tbl_entry **to,
		  uint32_t *out_tbl_ind)
{
	uint32_t ind;
	uint32_t first_3bytes = (uint32_t)ip->s6_addr[0] << 16 |
			ip->s6_addr[1] << 8 | ip->s6_addr[2];
	...
	if (depth <= 24) {
		...
		*out_tbl_ind = TBL24_IND;
		...
		/* last level/tbl8 */
		ind = ip->s6_addr[byte] & depth_to_mask_1b(depth);
		*from = &tbl[ind];
		ind += (1 << (8 - depth)) - 1;
		*to = &tbl[ind];
		*out_tbl_ind = tbl_ind;
 ...
 */
int
rte_lpm6_delete(struct rte_lpm6 *lpm, const struct in6_addr *ip, uint8_t depth)
{
	struct in6_addr masked_ip;
