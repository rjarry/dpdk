		...
		uint16_t portid, struct lcore_conf *qconf)
{
	...
	struct in6_addr ipv6_arr[nb_rx];
	uint16_t hops[nb_rx];
		...
		const uint8_t flags)
{
	...
	struct in6_addr ipv6_arr[MAX_PKT_BURST];
...
fib_process_event_vector(struct rte_event_vector *vec, uint8_t *type_arr,
			 struct in6_addr *ipv6_arr, uint64_t *hopsv4, uint64_t *hopsv6,
		      ...
		      const uint8_t flags)
{
	...
	ipv6_arr = (struct in6_addr *)&hops[evt_rsrc->vector_size];
