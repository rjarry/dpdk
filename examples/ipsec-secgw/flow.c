	...
	for (ti = 0; ti < n_tokens; ti++) {
		...
		if (strcmp(tokens[ti], "ipv6") == 0) {
			...
			if (strcmp(tokens[ti], "src") == 0) {
				...
				if (status->status < 0)
					return;
				if (ipv6_addr_cpy(rule->ipv6.spec.hdr.src_addr.s6_addr,
						  rule->ipv6.mask.hdr.src_addr.s6_addr,
						  tokens[ti], status))
					return;
			}
			if (strcmp(tokens[ti], "dst") == 0) {
				...
				if (status->status < 0)
					return;
				if (ipv6_addr_cpy(rule->ipv6.spec.hdr.dst_addr.s6_addr,
						  rule->ipv6.mask.hdr.dst_addr.s6_addr,
						  tokens[ti], status))
					return;
			}
			continue;
