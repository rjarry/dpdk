		...
		case RTE_FLOW_ITEM_TYPE_IPV6:
			...
			if (ipv6_spec && ipv6_mask) {
				...
				for (j = 0; j < ICE_IPV6_ADDR_LENGTH; j++) {
					if (ipv6_mask->hdr.src_addr.s6_addr[j]) {
						*input |= ICE_INSET_IPV6_SRC;
						break;
					}
				}
				for (j = 0; j < ICE_IPV6_ADDR_LENGTH; j++) {
					if (ipv6_mask->hdr.dst_addr.s6_addr[j]) {
				...
				for (j = 0; j < ICE_IPV6_ADDR_LENGTH; j++) {
					if (ipv6_mask->hdr.src_addr.s6_addr[j]) {
						f->src_addr[j] =
						ipv6_spec->hdr.src_addr.s6_addr[j];
						s->src_addr[j] =
						ipv6_mask->hdr.src_addr.s6_addr[j];
						input_set_byte++;
					}
					if (ipv6_mask->hdr.dst_addr.s6_addr[j]) {
						f->dst_addr[j] =
						ipv6_spec->hdr.dst_addr.s6_addr[j];
						s->dst_addr[j] =
						ipv6_mask->hdr.dst_addr.s6_addr[j];
						input_set_byte++;
					}
				}
				if (ipv6_mask->hdr.proto) {
