...
test_toeplitz_hash_calc(void)
{
	...
	for (i = 0; i < RTE_DIM(v6_tbl); i++) {
		/*Fill ipv6 hdr*/
		for (j = 0; j < RTE_DIM(ipv6_hdr.src_addr.s6_addr); j++)
			ipv6_hdr.src_addr.s6_addr[j] = v6_tbl[i].src_ip[j];
		for (j = 0; j < RTE_DIM(ipv6_hdr.dst_addr.s6_addr); j++)
			ipv6_hdr.dst_addr.s6_addr[j] = v6_tbl[i].dst_ip[j];
