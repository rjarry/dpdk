...
lookup_and_check_asc(struct rte_fib6 *fib,
	struct in6_addr *ip_arr,
	struct in6_addr *ip_missing, uint64_t def_nh,
	uint32_t n)
{
...
{
	uint64_t def_nh = 100;
	struct in6_addr ip_arr[RTE_IPV6_MAX_DEPTH];
	struct in6_addr ip_add = RTE_IPV6(0x8000, 0, 0, 0, 0, 0, 0, 0);
	struct in6_addr ip_missing =
	...
	for (i = 0; i < RTE_IPV6_MAX_DEPTH; i++) {
		...
		if (j < RTE_IPV6_ADDR_SIZE) {
			ip_arr[i].s6_addr[j] |= UINT8_MAX >> ((RTE_IPV6_MAX_DEPTH - i) % CHAR_BIT);
			for (j++; j < RTE_IPV6_ADDR_SIZE; j++)
				ip_arr[i].s6_addr[j] = 0xff;
		}
	}
