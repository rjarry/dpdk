...
static struct in6_addr ip6_addr = RTE_IPV6(0x2001, 0x0200, 0, 0, 0, 0, 0, 0);
		    ...
		    uint8_t fill_mode)
{
	...
	for (i = 0; i < nb_frags; i++) {
		...
		ip_hdr->src_addr.s6_addr[7] = (flow_id >> 16) & 0xf;
		ip_hdr->src_addr.s6_addr[7] |= 0x10;
		ip_hdr->src_addr.s6_addr[8] = (flow_id >> 8) & 0xff;
		ip_hdr->src_addr.s6_addr[9] = flow_id & 0xff;

		ip_hdr->dst_addr.s6_addr[7] = (flow_id >> 16) & 0xf;
		ip_hdr->dst_addr.s6_addr[7] |= 0x20;
		ip_hdr->dst_addr.s6_addr[8] = (flow_id >> 8) & 0xff;
		ip_hdr->dst_addr.s6_addr[9] = flow_id & 0xff;
