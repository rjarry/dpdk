 ...
 */
static inline int check_lpm6_rule(const struct in6_addr *ip,
	const struct in6_addr *ip_rule, uint8_t depth)
{
	int k;
	uint8_t mask;
	...
	for (k = 0; k < 16; k++) {
		if (depth >= 8) {
			if (ip->s6_addr[k] != ip_rule->s6_addr[k])
				return -1;
		} else if (depth > 0) {
			mask = (uint8_t)((unsigned int)(-1) << (8 - depth));
			if ((ip->s6_addr[k] & mask) == (ip_rule->s6_addr[k] & mask))
				return 0;
			else
				return -1;
		} else
 ...
 * else return -1;
 */
static int get_next_hop(const struct in6_addr *ip, uint8_t *next_hop,
...
static void generate_large_ips_table(int gen_expected_next_hop)
{
	...
	for (i = 0; i < NUM_IPS_ENTRIES; i++) {
		for (j = 0; j < 16; j++)
			large_ips_table[i].ip.s6_addr[j] = rte_rand();
	}

	for (k = j = 0, i = 0; i < NUM_IPS_ENTRIES; i++) {
		mask_ip6_prefix(large_ips_table[i].ip.s6_addr,
			large_route_table[j].ip.s6_addr, large_route_table[j].depth);
		k++;
